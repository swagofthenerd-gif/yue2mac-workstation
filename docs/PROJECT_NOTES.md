...
- **Clean inputs (2026-09-25).** `levo2_engine.py --stems` renders LeVo's vocal and band straight from the tokens, the way Tencent's gen_type vocal/bgm does (band stream → code 9670, vocal stream → 3142), with no separation. On the rock demo, Whisper heard 68/73 lyric words in the native vocal vs 63/73 in the separated one. Clean voice samples: GTSinger studio tenor and alto (CC BY-NC-SA; non-commercial only).
- **SoulX-Singer-SVC** (engine/soulx_swap.py, scripts/setup_soulx.sh; Apache-2.0, 2026): zero-shot singing conversion that transfers timbre *and style*. ~3 min per song on the M2 Ultra, vs ~14–22 min for Seed-VC. It outputs 24 kHz (nothing above 12 kHz). Never use its auto_shift, which shifts by arbitrary semitones and puts the vocal out of key; only whole octaves.
